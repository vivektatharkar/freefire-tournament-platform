...
/* ---------- styles (same as headshot, just reused) ---------- */
const pageBase = {
  minHeight: "100vh",
  padding: "40px 24px",
...
/* ---------- main CS page ---------- */
export default function TournamentsCS() {
  ...
  // load CS matches + joined ids
  useEffect(() => {
    async function load() {
      try {
        const res = await axios.get("http://localhost:5000/api/cs/tournaments");
        ...
        if (tkn) {
          const joined = await axios.get(
            "http://localhost:5000/api/cs/joined/my",
        ...
        console.error(err);
        setError(
          err.response?.data?.message || "Failed to load CS matches"
        );
      }
    }
    load();
  ...
  const confirmJoinYes = async (idStr, rawDate, entryFee) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/api/cs/${idStr}/join`,
        { team_side },
        { headers: { Authorization: `Bearer ${tkn}` } }
      );
      ...
      // update seats locally using API response if available
      const { joined_count, slots } = res.data || {};
      setMatches((prev) =>
        prev.map((m) =>
          String(m.__id ?? m.id) === String(idStr)
            ? {
                ...m,
                joined_count:
                  joined_count ?? Number(m.joined_count || 0) + 1,
                slots: slots ?? m.slots,
              }
            : m
        )
      ...
      } else if (msg === "Already joined CS match") {
        setCardMessage((s) => ({
          ...s,
          [idStr]: "ℹ️ You have already joined this match.",
        }));
  ...
  const fetchDetails = async (idStr) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/cs/${idStr}/details`,
        { headers: { Authorization: `Bearer ${tkn}` } }
      );
      setDetailsById((s) => ({
        ...s,
  ...
  const fetchParticipants = async (idStr) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/cs/${idStr}/participants`,
        { headers: { Authorization: `Bearer ${tkn}` } }
      );
      setParticipantsById((s) => ({
        ...s,
  ...
  const handleSaveTeamName = async (matchId, sideKey, name) => {
    ...
    try {
      const res = await axios.patch(
        `http://localhost:5000/api/cs/${matchId}/team-name`,
                  ...
                  <div style={buttonsRow}>
                    ...
                    {joined ? (
                      ...
                      <button
                        ...
                        disabled={
                          loadingJoin[idStr] || joinClosed || isFull
                        }
                      >
                        {loadingJoin[idStr]
                          ? "Joining..."
