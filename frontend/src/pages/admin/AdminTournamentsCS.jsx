...
export default function AdminTournamentsCS() {
  ...
  const loadMatches = useCallback(async () => {
    try {
      ...
      setError(e.response?.data?.message || "Failed to load CS tournaments.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  const toggleLock = async (t) => {
    ...
    try {
      await axios.patch(
        `http://localhost:5000/api/admin/cs/${t.id}/status`,
        { status: next },
        { headers: authHeaders() }
      );
      setItems((prev) =>
  ...
  const saveRoom = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/admin/cs/${roomModalId}/room`,
        roomForm,
        { headers: authHeaders() }
      );
      setItems((prev) =>
  ...
  const openPlayersModal = async (t) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/admin/cs/${t.id}/players`,
        { headers: authHeaders() }
      );
  ...
  return (
    <div style={pageBase}>
      <div style={container}>
        <div style={headerRow}>
          <h1 style={titleStyle}>Admin – Clash Squad CS</h1>
          <div style={walletBox}>
            <div style={bellDot} />
        ...
        <div style={flexRow}>
          ...
          <div style={listCard}>
            ...
            >
              <h3 style={{ margin: 0, fontSize: 16 }}>CS tournaments</h3>
