...
export default function AdminTournamentsB2B() {
  ...
  const loadTournaments = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const res = await axios.get("http://localhost:5000/api/admin/b2b", {
        headers: authHeaders(),
      });
      setItems(Array.isArray(res.data) ? res.data : []);
    } catch (e) {
      setError(e.response?.data?.message || "Failed to load B2B tournaments.");
    } finally {
      setLoading(false);
    }
  }, [authHeaders]);
  ...
  const saveRoom = async () => {
    try {
      await axios.patch(
        `http://localhost:5000/api/admin/b2b/${roomModalId}/room`,
        roomForm,
        { headers: authHeaders() }
      );
      setItems((prev) =>
  ...
  const openPlayersModal = async (t) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/admin/b2b/${t.id}/players`,
        { headers: authHeaders() }
      );
  ...
  return (
    <div style={pageBase}>
      <div style={container}>
        <div style={headerRow}>
          <h1 style={titleStyle}>Admin – B2B Tournaments</h1>
          <div style={walletBox}>
            <div style={bellDot} />
        ...
        <div style={flexRow}>
          ...
          <div style={listCard}>
            ...
            >
              <h3 style={{ margin: 0, fontSize: 16 }}>B2B tournaments</h3>
